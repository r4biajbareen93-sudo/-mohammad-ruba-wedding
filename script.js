...
(function () {
  ...
  /* ------------------------------------------------------------------
     بتلات ذهبية في الشاشة الأولى
  ------------------------------------------------------------------ */
  (function buildPetals() {
    var box = document.getElementById('petals');
    if (!box || reduce || lite) { return; }
    var n = window.innerWidth < 480 ? 8 : 12;
    var frag = document.createDocumentFragment();
    for (var i = 0; i < n; i++) {
      var el = document.createElement('span');
      var sc = (0.7 + Math.random() * 0.8).toFixed(2);
      el.className = 'petal';
      el.style.left = (Math.random() * 100).toFixed(1) + '%';
      el.style.scale = sc;
      el.style.animationDuration = (14 + Math.random() * 12).toFixed(1) + 's';
      el.style.animationDelay = (-Math.random() * 22).toFixed(1) + 's';
      el.style.setProperty('--px', (Math.random() * 120 - 60).toFixed(0) + 'px');
      el.style.setProperty('--pr', (Math.random() * 420 - 120).toFixed(0) + 'deg');
  ...
  function openInvitation() {
    ...
    var tRise = reduce ? 60   : 2050;   /* البطاقة تخرج وتظهر أمام الضيف */
    var tOpen = reduce ? 120  : 2750;   /* تُفتح البطاقة كما تُفتح باليد */
    var tZoom = reduce ? 200  : 4350;   /* تكبر البطاقة وتتحول إلى الدعوة */
    var tPage = reduce ? 220  : 4550;
    var tGone = reduce ? 320  : 4950;
